...
class MY_Model extends CI_Model
{
    ...
    public function _get_table_fields()
    {
        if (empty($this->table_fields)) {
            $table = $this->table;
            $schema_table = explode('.', $this->table);
            if (count($schema_table) > 1) {
                $table = $schema_table[1];
            }
            $this->table_fields = $this->_database->list_fields($table);
        }
        return true;
    }
    ...
    public function _prep_after_read($data, $multi = true)
    {
        ...
        if (isset($this->return_as_dropdown) && $this->return_as_dropdown == 'dropdown') {
            ...
            foreach ($data as $row) {
                ...
                $dropdown[$count] = '<option value=' . $row[$this->primary_key] . '>' . $row[$this->_dropdown_field] . '</option>';
                $count++;
            }
            $data = $dropdown;
    ...
    public function from_form($rules = null, $additional_values = null, $row_fields_to_update = array())
    {
        ...
        if (!isset($rules)) {
            ...
            if (array_key_exists('insert', $rules)) {
                $rules = $rules['insert'];
            }
        }
        ...
        foreach ($rules as $key => $rule) {
            if (is_array($rule['rules'])) {
                foreach ($rule['rules'] as $rule_type) {
                    if (is_string($rule_type)) {
                        $this->form_validation->set_rules($rule['field'], $rule['label'], $rule_type);
                        array_shift($rule['rules']);
                    }
                }
            }
        ...
        if ($this->form_validation->run()) {
            ...
            foreach ($rules as $key => $rule) {
                ...
                if (count($column) > 1) {
                    ...
                    for ($i = $length - 1; $i >= 0; $i--) {
                        $first_value = ($i == $length - 1);

                        $new_data = $first_value ?
                        array(
                            $column[$i] => $this->input->post($rule['field']),
                        ) :
                        array(
                            $column[$i] => $new_data,
                        );
                    ...
                    if (isset($this->validated[$column_name])) {
                        $this->validated[$column_name] = json_encode(
                            array_merge_recursive(
                                json_decode($this->validated[$column_name], true),
                    ...
                    $this->validated[$column_name] = json_encode($new_data[$column_name]);

                    continue;
                }

                if (in_array($key, $this->_can_be_filled)) {
                    $this->validated[$key] = $this->input->post($rule['field']);
                }
            }
            ...
            // print_r($this->_can_be_filled); echo '<br/> <br/> <br/> ';
            return $this;
        } else {
    ...
    public function update($data = null, $column_name_where = null, $escape = true)
    {
        if (!isset($data) && $this->validated != false) {
            $data = $this->validated;
            $this->validated = false;
        } else if (isset($data) && $this->validated != false) {
     ...
     * @param  bool    $escape            - if set to false, will turn escape query off
     * @return $this
     */
    public function where($field_or_array = null, $operator_or_value = null, $value = null, $with_or = false, $with_not = false, $custom_string = false, $escape = true)
    {
        ...
        if ($custom_string === true) {
            ...
            $this->_database->where($field_or_array, null, $escape);
            ...
            // print_r($field_or_array);exit;
            if (($field_or_array === 'not_in') || ($field_or_array === 'in')) {
                $this->_database->{$where_or . '_' . $field_or_array}($this->table . '.' . $operator_or_value[0], $operator_or_value[1]);
            } else {
    ...
    public function count_rows($where = null)
    {
        ...
        $number_rows = $this->_database->count_all_results($this->table, false);
        $this->_reset_trashed();
        $this->_database->reset_query();
        return $number_rows;
    }
    ...
    public function with($request, $arguments = array())
    {
        ...
        $requests = (strpos($request, '|')) ? explode('|', $request) : $request;
        if (is_array($requests)) {
            echo 'ooo';
            foreach ($requests as $request) {
                if (array_key_exists($request, $this->_relationships)) {
                    $this->_requested[$request] = array('request' => $request);
                    $this->_requested[$request]['parameters'] = array();
                }
            }
        } else if (array_key_exists($request, $this->_relationships)) {
            $this->_requested[$request] = array('request' => $request);
            $parameters = array();
                ...
                if (!empty($request['parameters'])) {
                    if (array_key_exists('fields', $request['parameters'])) {
                        if ($request['parameters']['fields'] == '*count*') {
                            ...
                            foreach ($fields as $field) {
                                $select[] = (strpos($field, '.') === false) ? '' . trim($field) . '' : trim($field);
                            }
                    ...
                    if (array_key_exists('limit', $request['parameters'])) {
                        $sub_results->limit($request['parameters']['limit']);
                    }
    ...
    public function query($data = null)
    {
        ...
        $this->_database->query("SET application_name = 'SYMFO@{$this->input->server('REMOTE_ADDR')}'");

        $result = $this->_database->query("SELECT {$data['name']}({$query})")->row_array()[$data['name']];
        return $this->_prep_after_read($result);
    }
     ...
     * @return mixed
     */
    public function sql($sql, $row = false)
    {
    ...
    public function clear_validated()
    {
        $this->validated = false;
    }
    ...
    {
        $sql = $this->_database->set(array_merge($this->validated, $data))->get_compiled_insert($this->table);
        return $sql;
    }
     ...
     * @return mixed
     */
    protected function pop_out($row, $key, $name, $skip)
    {
        $data = [];

        foreach ($name as $old_key => $field) {
            $new_key = is_array($field) ? array_values($field)[0] : $field;
            $field = is_array($field) ? array_keys($field)[0] : $field;
            ...
            if (is_object($row)) {
                ...
                $new_value = isset($row->{$key}->{$field}) ? $row->{$key}->{$field} : null;
     ...
     */
    protected function map($row, $fields, $skip)
    {
        // Holds the new mapped item per row
        $mapped_row = [];

        // Loop through each key-value pair in the item
        foreach ($row as $key => $value) {
            // Set the initial key as a default
            $name = $key;

            // Check if the key should be mapped
            ...
            if (is_array($name)) {
                foreach ($name as $index => $new_name) {
                    $pop_out_name = is_string($index) ? $index : $new_name;

                    if (isset($row->{$key}->{$pop_out_name})) {
                        $mapped_row[$new_name] = $row->{$key}->{$pop_out_name};
                    }
                }
            } else {

                // Check if the value is in json format and
                // set it on the new item holder
                $is_json = false;
                if (is_string($value)) {
                    $is_json = json_decode($value);
                    $value = $is_json ? $is_json : $value;
                }

                // Set new item holder with mapped name-value
                $mapped_row[$name] = $value;
            }

            if ((is_array($value) || is_object($value)) && !is_array($name)) {
                $data = $this->map($value, $fields, $skip);
                $mapped_row[$name] = $data;
                continue;
            }
        }

        // exit(print_r($mapped_row));
        return $mapped_row;
    }

    /**
     * Change each database column name to custom fields
     *
     * @access public
     * @param  array|object $data The fecthed data(rows or single row)
     * @return array|object The new mapped data
     */
    public function map_fields($data, $skip = true)
    {
        if (!isset($this->_map_fields)) {
            $class_name = get_class($this);
            show_error("ERROR: \$_map_fields variable is not found {$class_name} class.");
        }
        ...
        // Holds the new mapped data
        $mapped_data = [];

        // Check if the data is sequential array
        if (array_keys((array) $data) === range(0, count((array) $data) - 1)) {
            // Loop through each item in the array
            foreach ($data as $row) {
                // Set the new converted item into the mapped data holder
                $mapped_data[] = $this->map($row, $this->_map_fields, $skip);
            }
        } else {
            // Set the new converted item to mapped data
            $mapped_data = $this->map($data, $this->_map_fields, $skip);
        }
    ...
    /**
     * Join statement on the database connection
     *
     * @access public
     * @param  string  $table    table name
     * @param  string  $cond
     * @param  string  $type     join type; options are: left, right, outer, inner, left outer, and right outer
     * @param  bool    $escape
     * @return mixed
     */
    public function join($table, $cond, $type = 'left', $escape = null)
    {
